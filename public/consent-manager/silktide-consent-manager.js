...
class SilktideConsentManager {
  ...
  /**
   * Trigger Matomo tracking consent updates.
   * @param {boolean|null} accepted - Whether consent is accepted
   */
  triggerMatomoConsent(accepted) {
  ...
  /**
   * Trigger consent integrations based on consent type configuration.
   */
  triggerConsentIntegration(consentType, accepted) {
    const hasGtagIntegration = !!consentType.gtag;
    const hasMatomoIntegration = !!consentType.matomo;

    if (!hasGtagIntegration && !hasMatomoIntegration) {
      return;
    }

    if (hasGtagIntegration) {
    ...
    if (hasMatomoIntegration) {
      this.triggerMatomoConsent(accepted);
    }
  ...
  batchUpdateConsents(consentStates) {
    ...
    // Second pass: save to localStorage and run non-gtag integrations
    changes.forEach(({ type, newState }) => {
      this.setConsentChoice(type.id, newState);

      if (type.matomo) {
        this.triggerMatomoConsent(newState);
      }
    });
  ...
  runConsentCallbacksOnLoad() {
    ...
    this.config.consentTypes.forEach((type) => {
      ...
      if (type.required) {
        ...
        if (type.matomo) {
          this.triggerMatomoConsent(true);
        }
      ...
      // Check if rejected
        ...
        if (type.matomo) {
          this.triggerMatomoConsent(false);
        }

    ...
    <header className="fixed inset-x-0 top-0 z-50 px-3 pt-3 sm:px-5 sm:pt-4">
      ...
      >
        <div className="relative flex h-[64px] items-center justify-between px-4 sm:px-5 lg:px-6">
          ...
          <div className="hidden lg:absolute lg:inset-0 lg:flex lg:items-center lg:justify-center pointer-events-none">
            <div className="flex items-center gap-7 pointer-events-auto">
              {links.map((link) => (
                <Link key={link.label} href={link.href} className="text-[15px] font-medium text-black transition hover:text-[#6d49fd]">
                  {link.label}
                </Link>
              ))}
            </div>
          ...
          <div className="flex items-center gap-2 z-10">
            ...
            <button onClick={() => setOpen(!open)} className="grid h-10 w-10 place-items-center rounded-full border border-[#ded8eb] bg-white text-[#332d40] lg:hidden active:scale-[0.97] transition-transform duration-150" aria-expanded={open} aria-label={open ? "Close navigation" : "Open navigation"}>
        ...
        <AnimatePresence initial={false}>
          {open && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25 }} className="overflow-hidden lg:hidden">
              <div className="border-t border-[#e5dfef] px-4 pb-5 pt-3">
                <div className="flex flex-col">
                  {links.map((link, index) => (
                    <motion.div key={link.label} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * .035 }}>
                      <Link href={link.href} onClick={() => setOpen(false)} className="flex items-center justify-between border-b border-[#ebe7f2] py-4 text-[15px] font-medium text-black">
                        {link.label}<span className="text-[#9b93a8]">0{index + 1}</span>
                      </Link>
                    </motion.div>
                  ))}
                </div>
